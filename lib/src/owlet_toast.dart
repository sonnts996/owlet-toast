...
abstract class OwletToast<T extends Object, E extends Object?> {
  ...
  const OwletToast({required this.overlayManager});
  ...
  /// The [type] lets you know which type of the toast (such as information, error, warning),
  /// and the [data] is the toast's user data.
  ///
  ...
  Future<R?> show<R extends Object>({
    ...
    overlayManager.show<R?>(
      builder: (context, entry) => ToastWidget(
        ...
        entry: entry,
        transitionDelegate: transitionDelegate ?? TranslateTransitionDelegate(direction: alignment),
        alignment: alignment,
      ),
